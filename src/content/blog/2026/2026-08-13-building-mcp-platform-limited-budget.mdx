...
Build one small Agent Gateway. Put Azure API Management in front of it. Use domain-owned MCP servers behind it. Use MLflow from [mlflow.org](https://mlflow.org/) for traces, evaluation, and feedback. Use the platform to make the path from business need to governed tool call shorter.
...
## MLflow.org is useful, but not as magic glue
...
The official MLflow MCP server from mlflow.org is aimed at interacting with MLflow traces programmatically. The docs also mark that MCP feature as experimental. That is useful, but it is not the same thing as a complete enterprise Agent Gateway.
...
- MLflow from mlflow.org for tracing, evaluation, and improvement loops
...
- MLflow OSS from mlflow.org with a database backend and artifact storage
- APIM in front
