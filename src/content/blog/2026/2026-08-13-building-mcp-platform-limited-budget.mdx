...
description: "AI can generate more code. That is not the bottleneck. The real work is removing integration, ownership, approval, and governance bottlenecks with a domain-driven MCP platform, APIM, and MLflow from mlflow.org."
...
featured: true
source: "Architecture follow-up on domain-driven MCP, Azure API Management, Agent Gateway, and MLflow from mlflow.org"
---
...
Build one small Agent Gateway. Put Azure API Management in front of it. Use domain-owned MCP servers behind it. Use MLflow from [mlflow.org](https://mlflow.org/) for AI Gateway, traces, evaluation, and feedback. Use the platform to make the path from business need to governed tool call shorter.
...
- MLflow, the open-source project from [mlflow.org](https://mlflow.org/), removes the "how do we learn from real runs without guessing?" bottleneck.
...
flowchart LR
  ...
  mlflow["MLflow<br/>mlflow.org<br/>traces + evals"]
  llm["MLflow AI Gateway<br/>model routing + cost"]
...
## Use MLflow AI Gateway, but do not confuse it with the domain gateway

MLflow, the open-source project documented at [mlflow.org](https://mlflow.org/), is the part I would use carefully.

Actually, I would use more of it than a first sketch might suggest.

MLflow's AI Gateway is useful for exactly the kind of budget-constrained setup this article is about. It gives you a unified endpoint for model providers, centralized key handling, endpoint configuration, usage tracking, budget controls, guardrails, and traces without asking a small team to build that plumbing from scratch.

So yes: use the MLflow AI Gateway for the model/provider side.

But I would still keep a thin domain Agent Gateway in front of the MCP execution path.

Those are different jobs.

MLflow AI Gateway answers:

- which model/provider endpoint should this request use?
- how do we centralize model credentials?
- how do we track usage, latency, token count, and cost?
- how do we apply model-side guardrails and budget controls?

The domain Agent Gateway answers:

- which business action is being attempted?
- which user or service identity is the agent acting for?
- which domain MCP tools are available in this context?
- does this action need approval before execution?
- what idempotency, audit, and rollback rules apply?
- which domain owns the tool contract when it fails?

If the platform only needs to host a simple agent endpoint, MLflow Agent Server may be enough to start. It can expose an agent as a FastAPI-backed endpoint with validation, streaming support, and built-in tracing. That is useful.

But for enterprise MCP, the hard part is usually not serving one agent. The hard part is governing tool access across domains without turning the platform team into the owner of every business rule.

That is why the domain Agent Gateway should stay small, and why the model gateway should come from MLflow instead of custom code.

MLflow is also strongest as the evidence layer around the platform:
...
The official MLflow MCP server from mlflow.org is aimed at interacting with MLflow traces programmatically. That is useful, but it is not the same thing as a complete enterprise domain gateway for MCP tool entitlement, approvals, and business-action routing.

So the better budget architecture is:

- APIM as the enterprise front door
- MLflow AI Gateway for model/provider routing, credentials, usage, cost, and guardrails
- a small custom Agent Gateway for domain runtime policy and MCP orchestration
- MLflow tracing/evaluation from mlflow.org for improvement loops
- domain MCP servers for owned capabilities

This gives you a learning system without pretending the model gateway is the whole domain runtime.
...
sequenceDiagram
  ...
  participant LLM as MLflow AI Gateway
  ...
  AG->>LLM: classify task + plan through governed model route
...
- one MCP server
- MLflow AI Gateway endpoint for the approved model/provider route
- MLflow trace logging, using the mlflow.org project rather than a custom trace store
...
- MLflow OSS from mlflow.org with AI Gateway enabled, a database backend, and artifact storage
- APIM in front
...
Do not build your own model gateway before checking whether MLflow AI Gateway already covers the need.

Do not let MLflow become an excuse to skip domain audit design.
...
- [MLflow AI Gateway for coding and long-running agents](https://mlflow.org/docs/latest/genai/governance/ai-gateway/coding-agents/)
