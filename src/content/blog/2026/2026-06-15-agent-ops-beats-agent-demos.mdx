...
The fix is not to keep shoving more text into the same conversation. The fix is to create a proper handoff before the session gets heavy.
...
The point is that a fresh session should be able to continue the work without replaying the entire chat history like a courtroom transcript.
